...
describe('Learning Materi Module', () => {
  beforeEach(() => {
    loginByUi();
    cy.visit('/belajar-saya/materi');
    cy.get(tid('learning.materi.page')).should('be.visible');
  });

  it('TC-MATERI-01 - Buka materi', () => {
    const materiId = getEnv('TEST_MATERI_ID');
    cy.get(tid(`learning.materi.item.${materiId}`)).should('be.visible');
    cy.get(tid(`learning.materi.item.${materiId}.open`)).click();

    cy.get(`${tid('learning.materi.content')}, ${tid('learning.materi.player')}`)
      .should('be.visible');
  });
  ...
  it('TC-MATERI-03 - Tandai materi selesai / progress berubah', () => {
    ...
    cy.get('body').then(($body) => {
      if ($body.find(tid('learning.materi.complete')).length) {
        cy.get(tid('learning.materi.complete')).click();
        cy.get(`${tid('common.toast.message')}, ${tid('learning.materi.success')}`).should('be.visible');
      } else {
        cy.get(`${tid('learning.materi.content')}, ${tid('learning.materi.player')}`).should('be.visible');
      }
    });
  });
});
